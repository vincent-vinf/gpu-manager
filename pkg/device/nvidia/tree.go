...
func printNode(node *NvidiaNode) string {
	if node.ntype != nvml.TOPOLOGY_INTERNAL {
		return fmt.Sprintf("%s (aval: %d, id: %d, pids: %+v, usedMemory: %d, totalMemory: %d, allocatableCores: %d, allocatableMemory: %d)\n",
			node.String(), node.Available(), node.Meta.ID, node.Meta.Pids, node.Meta.UsedMemory, node.Meta.TotalMemory,
			node.AllocatableMeta.Cores, node.AllocatableMeta.Memory)
	}

	return fmt.Sprintf("%s (id: %d, pids: %+v, usedMemory: %d, totalMemory: %d, allocatableCores: %d, allocatableMemory: %d)\n",
		node.String(), node.Meta.ID, node.Meta.Pids, node.Meta.UsedMemory, node.Meta.TotalMemory,
		node.AllocatableMeta.Cores, node.AllocatableMeta.Memory)
}
